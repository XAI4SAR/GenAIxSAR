...
writer = SummaryWriter('../tensorboard/'+'WGANGP')
    ...
    for epoch in range(train_epoch):
        ...
        for idx, data in enumerate(train_loader):
            ...
            for d_index in range(d_steps):
                ...
                D_real_loss = torch.mean(decison_real)
                ...
                D_fake_loss = torch.mean(decison_fake)
                C_fake_loss = Class_loss(class_fake, real_label)
                A_fake_loss = Az_loss(theta_fake, real_az_vec)

                gradient_penalty = calc_gradient_penalty(mini_batch,D, x_data.data, x_re)
                D_train_loss = ( D_fake_loss - D_real_loss + gradient_penalty) + 5*(C_real_loss + C_fake_loss) + 20*(A_real_loss + A_fake_loss)
