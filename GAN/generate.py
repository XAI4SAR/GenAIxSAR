...
# nohup python generate_64.py > G_result.txt
...
G = Generator()
G.cuda()
G.load_state_dict(torch.load('.acgan_code/MSTAR/MSTAR_ACGAN_GP_generator_param.pkl')) #加载预训练好的模型权重
D = Discriminator()
D.cuda()
D.load_state_dict(torch.load('.acgan_code/MSTAR/MSTAR_ACGAN_GP_discriminator_param.pkl')) #加载预训练好的模型权重
...
for idx, data in enumerate(train_loader):
    ...
    d_fake, class_fake, theta_pre = D(x_re)
    decison_fake = d_fake.squeeze()
    class_fake = class_fake.squeeze()
    theta_pre = theta_pre.squeeze()
